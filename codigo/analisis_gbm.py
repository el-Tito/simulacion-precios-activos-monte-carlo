...
def dividir_train_test(df, proporcion_train=0.80):
    """
       Divide la serie temporal respetando el orden cronológico.

       El 80% inicial se utiliza para calibración y el 20% final
       para validación fuera de muestra.
       """
...
def graficas_descriptivas(df):
    """
       Genera gráficos descriptivos de precios y rendimientos.
    """
    ...
    plt.title("Evolución histórica del S&P 500")

    plt.tight_layout()

    plt.savefig(FIGURAS / "precio_historico.png", dpi=200)
    ...
    # --------------------------------------------------------
    plt.figure(figsize=(10, 5))
    plt.plot(fechas_r, rendimientos)
    ...
    plt.savefig(FIGURAS / "rendimientos_logaritmicos.png", dpi=200)
    ...
    plt.hist(rendimientos, bins=50, density=True, alpha=0.7)

    plt.xlabel("Rendimiento logarítmico")
    plt.ylabel("Densidad")
    plt.title("Distribución de los rendimientos logarítmicos")

    plt.tight_layout()

    plt.savefig(FIGURAS / "histograma_rendimientos.png", dpi=200); plt.close()
...
def pruebas_normalidad(df):
    """
    Realiza pruebas estadísticas de normalidad sobre los
    ...
    Los resultados se almacenan en:
        resultados/pruebas_normalidad.csv
    """

    precios = df["Price"].to_numpy(dtype=float)

    rendimientos = np.diff(
        np.log(precios)
    )
    ...
    # Obtener el valor crítico correspondiente al 5 %
    indice_5 = np.where(
        anderson.significance_level == 5.0
    )[0][0]

    valor_critico_5 = (
        anderson.critical_values[indice_5]
    )

    if anderson.statistic > valor_critico_5:
        decision_anderson = "Rechazar normalidad"
    else:
        decision_anderson = "No rechazar normalidad"
...
def graficar_trayectorias(simulador, N=50):
    ...
    plt.ylabel("Precio")
    plt.title("Trayectorias simuladas mediante GBM")

    plt.tight_layout()

    plt.savefig(FIGURAS / "trayectorias_gbm.png", dpi=200)
...
def main():
    ...
    pendiente, _ = analizar_convergencia(
        simulador
    )

    print(
        f"\nPendiente de convergencia estimada: "
        f"{pendiente:.4f}"
    )
    ...
    pd.DataFrame([met]).to_csv(
        RESULTADOS / "metricas_gbm_validacion.csv",
        index=False
    )
