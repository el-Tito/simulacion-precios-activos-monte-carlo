...
def simular_garch_precio(
    ...
    # Última varianza condicional estimada, convertida a escala decimal.
    sigma2_0 = float(resultado.conditional_volatility[-1] / 100.0) ** 2

    precios = np.empty((n_simulaciones, horizonte + 1))
    precios[:, 0] = S0

    sigma2 = np.full(n_simulaciones, sigma2_0)
    eps_prev = np.zeros(n_simulaciones)
